...
public class ChessGame {
	...
	private ChessPiece.PieceColor currentPlayer;
	...
	private void endTurn() {
		currentPlayer = (currentPlayer == ChessPiece.PieceColor.White) ? ChessPiece.PieceColor.Black
				: ChessPiece.PieceColor.White;
	}

	private boolean isCheckPreventable(ChessPiece.PieceColor color) {
	...
	private boolean isColorCheckMate(ChessPiece.PieceColor color) {
		if (!isKingCheck(color))
			return false;
		return !isCheckPreventable(color);
	}

	private boolean isKingCheck(ChessPiece.PieceColor kingColor) {
		Tuple kingLocation = board.getKingLocation(kingColor);
		return canOpponentTakeLocation(kingLocation, kingColor);
	}

	private boolean canOpponentTakeLocation(Tuple location, ChessPiece.PieceColor color) {
		ChessPiece.PieceColor opponentColor = ChessPiece.opponent(color);
	...
	private boolean isValidMoveForPiece(Tuple from, Tuple to) {
		...
		return repeatableMoves ? isValidMoveForPieceRepeatable(from, to) : isValidMoveForPieceNonRepeatable(from, to);
	}
	...
	public boolean isFirstMoveForPawn(Tuple from, ChessBoard board) {
		...
		if (tile.isEmpty() || tile.getPiece().getPieceType() != ChessPiece.PieceType.Pawn) {
			...
			ChessPiece.PieceColor color = tile.getPiece().getColor();
			return (color == ChessPiece.PieceColor.White) ? from.Y() == 6 : from.Y() == 1;
		}
	}
}
